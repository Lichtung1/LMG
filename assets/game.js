class GameScene extends Phaser.Scene {
    ...
    preload() {
        ...
        this.load.image(ASSETS.FULLSCREEN_PROMPT_IMAGE.key, ASSETS.FULLSCREEN_PROMPT_IMAGE.file);
    }
    ...
    create() {
        ...
        // --- Fullscreen and Orientation Handling for Mobile ---
        // Check if it's not a desktop browser (i.e., likely mobile or tablet)
        // Ensure ASSETS.FULLSCREEN_PROMPT_IMAGE is defined in your constants.js
        if (!this.sys.game.device.os.desktop && ASSETS.FULLSCREEN_PROMPT_IMAGE) {
            // Create an image-based prompt for the user to tap
            const promptImage = this.add.image(
                this.cameras.main.centerX,
                this.cameras.main.centerY,
                ASSETS.FULLSCREEN_PROMPT_IMAGE.key // Use the key from your ASSETS constant
            ).setOrigin(0.5)
                .setScrollFactor(0)
                .setDepth(1000)        // Ensure it's on top
                .setInteractive();

            // Handler for when the user taps anywhere on the screen for the first time
            const handleFirstTap = () => {
                if (!this.scale.isFullscreen) {
                    this.scale.startFullscreen();
                }
                if (promptImage && promptImage.active) {
                    promptImage.destroy();
                }
                // Clean up listeners
                this.input.off('pointerdown', handleFirstTap);
                if (promptImage && promptImage.input) { // Check if promptImage still has input property
                    promptImage.off('pointerdown', handleImageTap);
                }
            };

            // Handler specifically for when the prompt image itself is tapped
            const handleImageTap = () => {
                ...
                // Clean up listeners
                this.input.off('pointerdown', handleFirstTap);
                if (promptImage && promptImage.input) {
                    promptImage.off('pointerdown', handleImageTap);
                }
            };

            this.input.on('pointerdown', handleFirstTap, this);
            promptImage.on('pointerdown', handleImageTap, this);
        ...
        } // end if (!this.sys.game.device.os.desktop && ASSETS.FULLSCREEN_PROMPT_IMAGE)
        ...
        this.fishFliesSwarm = this.add.group({ runChildUpdate: true }); // Re-initializing here, ensure it's intended if also done above
