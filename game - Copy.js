class GameScene extends Phaser.Scene {
    ...
    setupMapAndLayers() { 
        ...
        } else { console.warn(`Tiled object layer '${TILED_NAMES.LAYER_TRIGGERS}' not found.`); }

        // Optional: Warn if MothTriggerZone wasn't found
        if (!this.mothTriggerZoneRect) {
    ...
    createFishflySwarm(spawnX, spawnY, playerTarget) {
        ...
        console.log(`Creating ${numFishFlies} fish flies from (${spawnX},${spawnY}) targeting player.`);
    
        for (let i = 0; i < numFishFlies; i++) {
            const fishFly = this.fishFliesSwarm.create(spawnX, spawnY, FISHFLIES_CONFIG.TEXTURE_KEY);
            if (!fishFly) continue;
    
            const scale = Phaser.Math.FloatBetween(FISHFLIES_CONFIG.SIZE.min, FISHFLIES_CONFIG.SIZE.max) / FISHFLIES_CONFIG.SIZE.max;
    
            const individualOffsetX = Phaser.Math.FloatBetween(-10, 10);
            const individualOffsetY = Phaser.Math.FloatBetween(5, 10);
            ...
            fishFly.setData({
                ...
                offsetXFromPlayerHead: individualOffsetX,
                offsetYFromPlayerHead: individualOffsetY,
                emergeTargetX: playerTarget.x + individualOffsetX,
                emergeTargetY: playerTarget.y + individualOffsetY,
                currentFig8CenterX: spawnX,
                currentFig8CenterY: spawnY,
                baseRadiusX: Phaser.Math.FloatBetween(FISHFLIES_CONFIG.FIGURE8_RADIUS_X.min, FISHFLIES_CONFIG.FIGURE8_RADIUS_X.max),
                baseRadiusY: Phaser.Math.FloatBetween(FISHFLIES_CONFIG.FIGURE8_RADIUS_Y.min, FISHFLIES_CONFIG.FIGURE8_RADIUS_Y.max),
                figure8Speed: Phaser.Math.FloatBetween(FISHFLIES_CONFIG.FIGURE8_SPEED.min, FISHFLIES_CONFIG.FIGURE8_SPEED.max) * Phaser.Math.RND.pick([-1, 1]),
                currentAngle: Phaser.Math.FloatBetween(0, FISHFLIES_CONFIG.FIGURE8_PHASE_OFFSET_MAX),
                radiusAmplitude: Phaser.Math.FloatBetween(FISHFLIES_CONFIG.FIGURE8_RADIUS_OSCILLATION.amplitude * 0.5, FISHFLIES_CONFIG.FIGURE8_RADIUS_OSCILLATION.amplitude * 1.5),
                radiusOscSpeed: Phaser.Math.FloatBetween(FISHFLIES_CONFIG.FIGURE8_RADIUS_OSCILLATION.speed * 0.5, FISHFLIES_CONFIG.FIGURE8_RADIUS_OSCILLATION.speed * 1.5) * Phaser.Math.RND.pick([-1, 1]),
                yBobAmplitude: Phaser.Math.FloatBetween(FISHFLIES_CONFIG.Y_BOB_AMPLITUDE.min, FISHFLIES_CONFIG.Y_BOB_AMPLITUDE.max),
                yBobSpeed: Phaser.Math.FloatBetween(FISHFLIES_CONFIG.Y_BOB_SPEED.min, FISHFLIES_CONFIG.Y_BOB_SPEED.max) * Phaser.Math.RND.pick([-1, 1]),
                currentYBobAngle: Phaser.Math.FloatBetween(0, Math.PI * 2)
            });
    
            fishFly.setAlpha(Phaser.Math.FloatBetween(FISHFLIES_CONFIG.ALPHA.min, FISHFLIES_CONFIG.ALPHA.max))
                .setScale(scale)
                .setDepth(playerTarget.depth + FISHFLIES_CONFIG.DEPTH_OFFSET_FROM_PLAYER)
                .setPipeline('Light2D'); // <-- ADD THIS LINE
    
            const emergeDuration = Phaser.Math.Between(FISHFLIES_CONFIG.EMERGE_DURATION.min, FISHFLIES_CONFIG.EMERGE_DURATION.max);
    
            this.tweens.add({
                targets: fishFly,
                x: fishFly.getData('emergeTargetX'),
                y: fishFly.getData('emergeTargetY'),
                duration: emergeDuration,
                ease: 'Quad.easeOut',
                onComplete: () => {
                    if (fishFly.active) {
                        fishFly.setData('state', 'buzzing');
                        fishFly.setData('currentFig8CenterX', fishFly.x);
                        fishFly.setData('currentFig8CenterY', fishFly.y);
                        fishFly.setData('currentAngle', Phaser.Math.FloatBetween(0, FISHFLIES_CONFIG.FIGURE8_PHASE_OFFSET_MAX));
                    }
                }
            });
        }
    ...
    updateFishFlies(time, delta) {
        if (!this.fishFliesSwarm) return;

        this.fishFliesSwarm.children.iterate(fishFly => {
            if (fishFly && fishFly.active && fishFly.getData) {
                const state = fishFly.getData('state');
                const playerTarget = fishFly.getData('playerTarget');

                if (!playerTarget || !playerTarget.active) {
                    // Optional: fishFly.destroy(); or fade out
                    return;
                }

                if (state === 'buzzing') {
                    // --- BUZZING STATE: Figure 8 with lagged follow ---
                    const desiredFig8CenterX = playerTarget.x + (fishFly.getData('offsetXFromTarget') || 0); // Ensure offsetX/Y exist or default
                    const desiredFig8CenterY = playerTarget.y + 32 + (fishFly.getData('offsetYFromTarget') || (-PLAYER_CONFIG.SPRITE_HEIGHT / 2 + 5));


                    let currentFig8CenterX = fishFly.getData('currentFig8CenterX');
                    let currentFig8CenterY = fishFly.getData('currentFig8CenterY');

                    // LERP the actual center towards the desired center
                    const lerpFactor = FISHFLIES_CONFIG.FOLLOW_LERP_FACTOR;
                    currentFig8CenterX += (desiredFig8CenterX - currentFig8CenterX) * lerpFactor;
                    currentFig8CenterY += (desiredFig8CenterY - currentFig8CenterY) * lerpFactor;

                    fishFly.setData('currentFig8CenterX', currentFig8CenterX);
                    fishFly.setData('currentFig8CenterY', currentFig8CenterY);

                    // Figure 8 calculation (from previous logic)
                    let currentAngle = fishFly.getData('currentAngle');
                    const figure8Speed = fishFly.getData('figure8Speed');
                    const baseRadiusX = fishFly.getData('baseRadiusX');
                    const baseRadiusY = fishFly.getData('baseRadiusY');
                    const radiusAmplitude = fishFly.getData('radiusAmplitude');
                    const radiusOscSpeed = fishFly.getData('radiusOscSpeed');
                    const radiusMultiplier = 1 + Math.sin(time * radiusOscSpeed) * radiusAmplitude / Math.max(baseRadiusX, baseRadiusY, 1);
                    const currentRadiusX = baseRadiusX * radiusMultiplier;
                    const currentRadiusY = baseRadiusY * radiusMultiplier;

                    currentAngle += figure8Speed * (delta / 16.667); // Normalize speed
                    fishFly.setData('currentAngle', currentAngle);

                    const localFig8X = currentRadiusX * Math.cos(currentAngle);
                    const localFig8Y = currentRadiusY * Math.sin(2 * currentAngle);

                    // Y-bobbing
                    const yBobAmplitude = fishFly.getData('yBobAmplitude');
                    const yBobSpeed = fishFly.getData('yBobSpeed');
                    let currentYBobAngle = fishFly.getData('currentYBobAngle');
                    currentYBobAngle += yBobSpeed * (delta / 16.667);
                    fishFly.setData('currentYBobAngle', currentYBobAngle);
                    const yBobOffset = Math.sin(currentYBobAngle) * yBobAmplitude;

                    fishFly.x = currentFig8CenterX + localFig8X;
                    fishFly.y = currentFig8CenterY + localFig8Y + yBobOffset;

                } else if (state === 'emerging') {
                    // Movement is handled by the tween.
                    // We could add subtle independent animation here if desired, like a slight wobble.
                }
            }
        });
    }
    ...
    // --- Update Method ---
    update(time, delta) {
        if (!this.cursors || !this.player?.active) return;

        // --- Update Parallax for ALL Backgrounds (Visible or Not) ---
        const scrollX = this.cameras.main.scrollX;
        // Use optional chaining ?. just in case creation failed
        //this.originalBackground?.setTilePosition(scrollX * PARALLAX_FACTORS.ORIGINAL_BG);
        ...
        this.updateStars(time); // Update star twinkle
        this.updateFireflies(time, delta);
        this.updateFishFlies(time, delta); 
        ...
        if (this.fishFliesSwarmLight) {
            ...
            if (activeFishFlies.length > 0) {
                ...
                // Restore intensity if it was set to 0 (adjust value from your config)
                const targetIntensity = FISHFLIES_CONFIG.LIGHT_INTENSITY || 0.55;
                if (this.fishFliesSwarmLight.intensity < targetIntensity) {
                    this.fishFliesSwarmLight.setIntensity(targetIntensity);
                }

            } else {
                // No active fishflies, turn off the light (or make it very dim)
        ...
        this.birds?.children.iterate(bird => {
            if (bird?.active && bird.update) bird.update(this.player, time, delta, worldView);
        });
        ...
        this.handleGlitches(time);

        // Update UI Manager
        this.uiManager?.handleInput();
        const uiData = { /* ... pass data ... */ };
        ...
        this.glitchTriggerZones.forEach(zone => {
            if (zone?.isActive && zone.rect && Phaser.Geom.Intersects.RectangleToRectangle(playerBodyRect, zone.rect)) {
                ...
                // LAKETRIGGER GLITCH
                    ...
                    // --- End of originalBackground flash ---


                    // --- PART 2: Other Lake Trigger Effects (message, pushback) ---

                    // 2a. Optional: Other minor glitches on the default background (bgLayer3) if desired.
                    // These would run while originalBackground is also storming.
                    // if (this.glitchManager && typeof this.glitchManager.triggerGlitchByKey === 'function') {
                    //     if (this.glitchManager.background === this.bgLayer3) { // Check if target is set back or was already bgLayer3
                    //        this.bgLayer3?.setVisible(true);
                    //        this.glitchManager.triggerGlitchByKey(Phaser.Input.Keyboard.KeyCodes.U);
                    //     }
                    // }

                    // 2b. Display "the lake is not safe right now" message
                    ...
                    // --- PART 3: Cooldown for this re-triggerable Lake Zone ---
                    ...
                    if (this.uiManager && typeof this.uiManager.startDialogueSequence === 'function') {
                        ...
                        this.uiManager.startDialogueSequence('fish_flies_fountain_intro', () => {
                            ...
                            // --- Make fishflies return home ---
                            if (this.fishFliesSwarm) {
                                this.fishFliesSwarm.children.each(fishFly => {
                                    if (fishFly && fishFly.active && fishFly.getData('state') !== 'returning_home') {
                                        const homeX = fishFly.getData('initialSpawnX');
                                        const homeY = fishFly.getData('initialSpawnY');
                        
                                        if (homeX !== undefined && homeY !== undefined) {
                                            fishFly.setData('state', 'returning_home');
                                            // Stop any existing tweens on this fishfly (like the emerge tween if it was interrupted)
                                            this.tweens.killTweensOf(fishFly);
                        
                                            this.tweens.add({
                                                targets: fishFly,
                                                x: homeX +16+ Phaser.Math.FloatBetween(-FISHFLIES_CONFIG.SIZE.max * 2, FISHFLIES_CONFIG.SIZE.max * 2), // Scatter them a bit at home
                                                y: homeY +16+ Phaser.Math.FloatBetween(-FISHFLIES_CONFIG.SIZE.max * 2, FISHFLIES_CONFIG.SIZE.max * 2),
                                                duration: Phaser.Math.Between(500, 750), // Duration for the return journey
                                                ease: 'Quad.easeIn',
                                                onComplete: () => {
                                                    if (fishFly.active) {
                                                        // Destroy fishflys
                                                        ...
                                                        // Optional: Check if all flies are done
                                                        if (this.fishFliesSwarm.countActive(true) === 0) {
                                                            console.log("All fishflies have returned home and are inactive.");
                                                            // You might want to clear the group if they won't be reused,
                                                            // or leave them inactive for potential respawn.
                                                            // this.fishFliesSwarm.clear(false, true); // Removes inactive children and destroys them
                                                        }
                                                    }
                                                }
                                            });
                                        } else {
                                            console.warn("Fishfly missing initialSpawnX/Y data, cannot return home. Removing.");
                                            if(fishFly.active) fishFly.destroy(); // Fallback
                                        }
                                    }
                                });
                            }
                            // --- End fishflies return home ---
                        });
                    } else {
